...
module.exports = class Users {
    ...
    async findTipoId(tipo) {
        ...
        const { rows } = await pool.query('SELECT id_tipo FROM tipo_usuario WHERE LOWER(tipo) = LOWER($1)', [tipo.trim()]);
        if (!rows[0]) {
    ...
    async deletar(id) {
        validarId(id);
        const { rowCount } = await pool.query('DELETE FROM usuarios WHERE id_user = $1', [id]);
        return rowCount > 0;
    }
};
